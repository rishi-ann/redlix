...
const Navbar = () => {
    ...
    return (
        <div className="fixed top-0 left-0 w-full z-50 px-4 py-4 sm:px-6">
            <nav
                className={`max-w-6xl mx-auto transition-all duration-300 rounded-full border border-gray-100 ${scrolled
                    ? "bg-white/90 backdrop-blur-md shadow-md py-2 px-6"
                    : "bg-white py-3 px-8 shadow-sm"
                    }`}
            >
                ...
                {mobileMenuOpen && (
                    <div className="md:hidden absolute top-20 left-4 right-4 bg-white border border-gray-100 rounded-3xl p-6 shadow-xl flex flex-col gap-4 animate-in fade-in zoom-in-95">
                        {navLinks.map((link) => (
                            <a
                                key={link.name}
                                href={link.href}
