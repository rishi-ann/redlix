...
export function DashboardClient({ initialCareers, initialMaintenanceRequests, initialSupportTickets }: { initialCareers: Career[], initialMaintenanceRequests: MaintenanceRequest[], initialSupportTickets: SupportTicket[] }) {
    ...
    const [activeView, setActiveView] = useState<"jobs" | "applications" | "maintenance" | "tickets">("jobs");
    ...
    const handleUpdateTicketStatus = async (id: string, newStatus: string) => {
        try {
            ...
            setSupportTickets(
                supportTickets.map((t) => (t.id === id ? { ...t, status: newStatus } : t))
            );
        } catch (error) {
        ...
        <div className="min-h-screen bg-gray-50 flex font-sans">
            ...
            `}>
                ...
                <nav className="mt-6 px-4 space-y-2">
                    ...
                    <button
                        onClick={() => { setActiveView("tickets"); setIsSidebarOpen(false); }}
                        className={`w-full flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 group ${activeView === "tickets"
                            ? "bg-red-500 text-white shadow-lg shadow-red-500/30"
                            : "text-gray-400 hover:bg-white/5 hover:text-white"
                            }`}
                    >
                        ...
                        {supportTickets.length > 0 && (
                            <span className={`ml-auto py-0.5 px-2 rounded-full text-xs font-bold ${activeView === "tickets" ? "bg-white text-red-500" : "bg-white/10 text-white"
                                }`}>
                    ...
                    {activeView === "tickets" && (
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <div className="mb-8">
                                ...
                                <p className="text-gray-500 mt-2">Manage ongoing client tickets and adjust their completion status.</p>
                            </div>
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                                {supportTickets.length > 0 ? (
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-left border-collapse">
                                            <thead>
                                                <tr className="bg-gray-50/50 border-b border-gray-100 text-xs text-gray-500 uppercase tracking-wider">
                                                    <th className="px-6 py-4 font-semibold">Client</th>
                                                    <th className="px-6 py-4 font-semibold">Ticket Context</th>
                                                    <th className="px-6 py-4 font-semibold text-center">Status</th>
                                                    <th className="px-6 py-4 font-semibold text-right">Progress Control</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-gray-100">
                                                {supportTickets.map((ticket) => (
                                                    <tr key={ticket.id} className="group hover:bg-gray-50/80 transition-colors">
                                                        <td className="px-6 py-4 align-top">
                                                            ...
                                                            <div className="text-xs text-gray-500 font-mono mt-0.5">ID: {ticket.clientId}</div>
                                                            <div className="text-xs text-gray-400 mt-1">By: {ticket.authorizedPerson}</div>
                                                        </td>
                                                        <td className="px-6 py-4 align-top max-w-sm">
                                                            <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold tracking-widest bg-gray-100 text-gray-600 mb-2 font-mono">
                                                                {ticket.ticketId}
                                                            </span>
                                                            <div className="text-xs text-blue-600 font-medium mb-1 break-all">
                                                                URL: {ticket.pageUrl}
                                                            </div>
                                                        ...
                                                        <td className="px-6 py-4 align-top text-center">
                                                            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-widest border ${ticket.status === 'OPEN' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                                                        ...
                                                        <td className="px-6 py-4 align-top text-right">
                                                            <select
                                                                ...
                                                                className="px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-medium text-gray-700 cursor-pointer focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all outline-none"
                                                            >
                                    ...
                                    <div className="text-center py-24">
                                        ...
                                        <p className="text-gray-900 text-lg font-semibold">No tickets yet</p>
                                        <p className="text-gray-500 text-sm mt-1">Ticket submissions will appear here securely.</p>
                                    </div>
                                )}
                            </div>
                        </div>
